...
REM Build the ABE helper first; native\src\abe.rs embeds it via include_bytes!.
set "HELPER_DIR=%PLUGIN_DIR%\native-abe-helper"
if exist "%HELPER_DIR%\Cargo.toml" (
  echo [build] cargo build --release --target=x86_64-pc-windows-msvc in %HELPER_DIR%
  pushd "%HELPER_DIR%"
  cargo build --release --target=x86_64-pc-windows-msvc
  if errorlevel 1 (
    echo [error] abe-helper build failed
    popd
    exit /b 1
  )
  popd
  copy /Y "%HELPER_DIR%\target\x86_64-pc-windows-msvc\release\abe-helper.exe" "%NATIVE_DIR%\abe-helper.exe" >nul
  if errorlevel 1 (
    echo [error] abe-helper copy failed
    exit /b 1
  )
)
...
for %%T in (%BUILD_TARGETS%) do (
  ...
  if "!TARGET_OS!"=="windows" (
    copy /Y "%NATIVE_DIR%\target\!RUST_TARGET!\release\xfill.dll" "!OUTFILE!" >nul
  ) else if "!TARGET_OS!"=="darwin" (
    copy /Y "%NATIVE_DIR%\target\!RUST_TARGET!\release\libxfill.dylib" "!OUTFILE!" >nul
  ) else (
    copy /Y "%NATIVE_DIR%\target\!RUST_TARGET!\release\libxfill.so" "!OUTFILE!" >nul
  )
  if errorlevel 1 (
...
set "ZIP_SOURCES='%PLUGIN_DIR%\config.json','%PLUGIN_DIR%\server.js'"
for %%T in (%BUILD_TARGETS%) do (
  for /f "tokens=1,2 delims=-" %%A in ("%%T") do (
    set "TARGET_OS=%%A"
    set "TARGET_ARCH=%%B"
